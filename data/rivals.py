# Character names, sidebar colors, emote IDs, and embed icons
characters = {
    'Zetterburn': {
        'color': 16742462,
        'emote': '<:Zetterburn:547189111052566556>',
        'icon': 'https://blair-c.github.io/roaa/img/zetterburn/icon.png'
    },
    'Forsburn': {
        'color': 11179322,
        'emote': '<:Forsburn:547189098859986984>',
        'icon': 'https://blair-c.github.io/roaa/img/forsburn/icon.png'
    },
    'Clairen': {
        'color': 16715114,
        'emote': '<:Clairen:547182529837400081>',
        'icon': 'https://blair-c.github.io/roaa/img/clairen/icon.png'
    },
    'Mollo': {
        'color': 16729927,
        'emote': '<:Mollo:937869128977510472>',
        'icon': 'https://blair-c.github.io/roaa/img/mollo/icon.png'
    },
    'Orcane': {
        'color': 4812504,
        'emote': '<:Orcane:547182506584047618>',
        'icon': 'https://blair-c.github.io/roaa/img/orcane/icon.png'
    },
    'Etalus': {
        'color': 6728666,
        'emote': '<:Etalus:547182495964069898>',
        'icon': 'https://blair-c.github.io/roaa/img/etalus/icon.png'
    },
    'Ranno': {
        'color': 379087,
        'emote': '<:Ranno:547182486971613185>',
        'icon': 'https://blair-c.github.io/roaa/img/ranno/icon.png'
    },
    'Hodan': {
        'color': 14024191,
        'emote': '<:Hodan:937869074778693632>',
        'icon': 'https://blair-c.github.io/roaa/img/hodan/icon.png'
    },
    'Kragg': {
        'color': 5866833,
        'emote': '<:Kragg:547182475835736155>',
        'icon': 'https://blair-c.github.io/roaa/img/kragg/icon.png'
    },
    'Maypul': {
        'color': 8439641,
        'emote': '<:Maypul:547182462015504404>',
        'icon': 'https://blair-c.github.io/roaa/img/maypul/icon.png'
    },
    'Sylvanos': {
        'color': 9082654,
        'emote': '<:Sylvanos:547182436161945601>',
        'icon': 'https://blair-c.github.io/roaa/img/sylvanos/icon.png'
    },
    'Olympia': {
        'color': 12091475,
        'emote': '<:Olympia:937869101697761291>',
        'icon': 'https://blair-c.github.io/roaa/img/olympia/icon.png'
    },
    'Wrastor': {
        'color': 10702538,
        'emote': '<:Wrastor:547182420462665738>',
        'icon': 'https://blair-c.github.io/roaa/img/wrastor/icon.png'
    },
    'Absa': {
        'color': 15372270,
        'emote': '<:Absa:547182403144384532>',
        'icon': 'https://blair-c.github.io/roaa/img/absa/icon.png'
    },
    'Elliana': {
        'color': 12151190,
        'emote': '<:Elliana:547182393908396052>',
        'icon': 'https://blair-c.github.io/roaa/img/elliana/icon.png'
    },
    'Pomme': {
        'color': 16504300,
        'emote': '<:Pomme:937869156085268531>',
        'icon': 'https://blair-c.github.io/roaa/img/pomme/icon.png'
    },
    'Ori': {
        'color': 15132415,
        'emote': '<:Ori:547182375956774920>',
        'icon': 'https://blair-c.github.io/roaa/img/ori/icon.png'
    },
    'Shovel Knight': {
        'color': 15774989,
        'emote': '<:ShovelKnight:547182360219746349>',
        'icon': 'https://blair-c.github.io/roaa/img/shovelknight/icon.png'
    }
}

# Rivals regions
regions = {
    'West Coast': {
        'abbreviation': 'WC',
        'emote': '<:WestCoast:547189520781803530>'
    },
    'Midwest': {
        'abbreviation': 'MW',
        'emote': '<:Midwest:547189509272633355>'
    },
    'East Coast': {
        'abbreviation': 'EC',
        'emote': '<:EastCoast:547189489861132308>'
    },
    'Europe': {
        'abbreviation': 'EU',
        'color': 3294334,
        'emote': '<:Europe:547189473432305665>',
        'icon': 'https://imgur.com/WPMrAhN.png'
    },
    'Australia': {
        'abbreviation': 'AU',
        'emote': '<:Australia:547189427416596502>'
    },
    'South America': {
        'abbreviation': 'SA',
        'emote': '<:SouthAmerica:547189406902124595>'
    },
    'Asia': {
        'abbreviation': 'AS',
        'emote': '<:Asia:547189391399976980>'
    },
    'Africa': {
        'abbreviation': 'AF',
        'emote': '<:Africa:547189379605725225>'
    },
}

# Rivals consoles
consoles = {
    'Nintendo Switch': {
        'color': 16711690,
        'emote': '<:switch:759539694937309186>',
        'icon': 'https://i.imgur.com/2ieAFjS.png'
    },
    'Xbox': {
        'color': 8304896,
        'emote': '<:xbox:759539695553085460>',
        'icon': 'https://i.imgur.com/6XdeiBK.png'
    },
    'Keyboard': {
        'color': 10070709,
        'emote': '⌨️',
        'icon': 'https://i.imgur.com/XYdnABI.png'
    }
}